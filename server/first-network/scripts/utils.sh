...
installChaincode2 () {
	...
	VERSION=${3:-1.0}
        set -x
	peer chaincode install -n mycc1 -v ${VERSION} -l ${LANGUAGE} -p github.com/chaincode/voting/user/ >&log.txt
	res=$?
        set +x
	cat log.txt
	verifyResult $res "Chaincode installation on peer${PEER}.org${ORG} has Failed"
...
instantiateChaincode2 () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc1 -l ${LANGUAGE} -v ${VERSION} -c '{"Args":[]}' -P "OR	('Org1MSP.peer','Org2MSP.peer')" >&log.txt
		res=$?
                set +x
	else
                set -x
		peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc1 -l ${LANGUAGE} -v 1.0 -c '{"Args":[]}' -P " OR ('Org1MSP.peer','Org2MSP.peer')" >&log.txt
		res=$?
                set +x
	fi
	cat log.txt
...
chaincodeQuery () {
  ...
  do
     ...
     set -x
     peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["queryAllVote","1"]}' >&log.txt
     echo $CHANNEL_NAME
	 res=$?
     set +x
...
chaincodeInvoke () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc -c '{"Args":["createVoting","1","first-vote","09/13/2018 6:20:00 PM","09/13/2018 8:20:00 PM"]}' >&log.txt
		res=$?
                set +x
	else
                set -x
		peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -c '{"Args":["createVoting","1","first-vote","09/13/2018 6:20:00 PM","09/13/2018 8:20:00 PM"]}' >&log.txt
		res=$?
                set +x
	fi
	cat log.txt
